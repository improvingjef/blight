import 'package:analyzer/dart/ast/ast.dart';
import 'package:analyzer/dart/ast/token.dart';
import 'package:analyzer/dart/ast/visitor.dart';

import '../mutant.dart';
import 'operator.dart';

/// Changes comparison boundaries:
/// < -> <=, > -> >=, <= -> <, >= -> >
///
/// This is distinct from the relational operator which swaps between
/// comparisons (e.g. == <-> !=). This operator specifically targets
/// off-by-one boundary errors.
class ConditionalBoundaryMutationOperator extends MutationOperator {
  @override
  String get name => 'conditional_boundary';

  static const Map<TokenType, TokenType> _replacements = {
    TokenType.LT: TokenType.LT_EQ,
    TokenType.GT: TokenType.GT_EQ,
    TokenType.LT_EQ: TokenType.LT,
    TokenType.GT_EQ: TokenType.GT,
  };

  @override
  List<Mutant> generate(
      CompilationUnit unit, String filePath, String source) {
    final visitor = _ConditionalBoundaryVisitor(filePath, source);
    unit.visitChildren(visitor);
    return visitor.mutants;
  }
}

class _ConditionalBoundaryVisitor extends RecursiveAstVisitor<void> {
  final String filePath;
  final String source;
  final List<Mutant> mutants = [];

  _ConditionalBoundaryVisitor(this.filePath, this.source);

  @override
  void visitBinaryExpression(BinaryExpression node) {
    final operatorToken = node.operator;
    final replacement = ConditionalBoundaryMutationOperator
        ._replacements[operatorToken.type];

    if (replacement != null) {
      final originalLexeme = operatorToken.lexeme;
      final mutatedLexeme = replacement.lexeme;

      mutants.add(Mutant(
        filePath: filePath,
        lineNumber: _lineNumberOf(source, operatorToken.offset),
        originalCode: originalLexeme,
        mutatedCode: mutatedLexeme,
        operatorName: 'conditional_boundary',
        description: 'changed boundary $originalLexeme to $mutatedLexeme',
        offset: operatorToken.offset,
        length: operatorToken.length,
      ));
    }

    super.visitBinaryExpression(node);
  }
}

int _lineNumberOf(String source, int offset) {
  var line = 1;
  for (var i = 0; i < offset && i < source.length; i++) {
    if (source[i] == '\n') line++;
  }
  return line;
}
